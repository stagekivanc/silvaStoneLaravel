...
@section('title', $isEdit ? 'Renk Düzenle' : 'Yeni Renk')
@section('page_title', $isEdit ? 'Renk Düzenle' : 'Yeni Renk')

@section('content')
<div class="max-w-3xl mx-auto">
    <div class="mb-8">
        <a href="{{ route('yonetim.urun-renkleri.index') }}" class="flex items-center gap-2 text-slate-400 hover:text-slate-900 text-xs font-bold uppercase tracking-widest">
    ...
    <form action="{{ $isEdit ? route('yonetim.urun-renkleri.update', $item->id) : route('yonetim.urun-renkleri.store') }}" method="POST" class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-2 gap-4">
            <div class="space-y-2">
                ...
                <input type="text" name="slug" value="{{ old('slug', $item->slug ?? '') }}" class="w-full bg-slate-50 border border-slate-200 py-3.5 px-5 rounded-xl font-mono text-sm">
            </div>
            <div class="space-y-2">
                <label class="text-[11px] font-bold text-slate-400 uppercase tracking-widest ml-1">Hex</label>
                <input type="text" name="hex" value="{{ old('hex', $item->hex ?? '#cccccc') }}" class="w-full bg-slate-50 border border-slate-200 py-3.5 px-5 rounded-xl font-mono text-sm" placeholder="#e8dcc8">
            </div>
        </div>
