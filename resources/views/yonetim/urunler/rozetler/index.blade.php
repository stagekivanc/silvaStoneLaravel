...
@section('title', 'Ürün Rozetleri')
@section('page_title', 'Ürün Rozetleri')
...
<div class="max-w-5xl mx-auto">
    ...
    <div class="flex justify-end mb-6">
        <a href="{{ route('yonetim.urun-rozetleri.create') }}" class="flex items-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-colors shadow-lg shadow-blue-600/20">
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-50">
                    ...
                    <tr class="hover:bg-slate-50/50">
                        ...
                        <td class="px-8 py-5 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('yonetim.urun-rozetleri.edit', $item->id) }}" class="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg"><i data-lucide="pencil" class="w-4 h-4"></i></a>
                                <form action="{{ route('yonetim.urun-rozetleri.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Silmek istediğinize emin misiniz?')">
                                    @csrf @method('DELETE')
                ...
                @empty
                    <tr><td colspan="5" class="px-8 py-16 text-center text-slate-400 text-sm">Henüz rozet yok.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
